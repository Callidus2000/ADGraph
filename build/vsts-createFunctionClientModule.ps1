...
$parentModule = 'ADGraph'
if (-not $ModuleName) { $ModuleName = 'ADGraph.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ADGraph'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ADGraph.psd1"
$prereqHash = @{
