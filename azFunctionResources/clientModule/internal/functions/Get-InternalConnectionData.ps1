...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ADGraph.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }

		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ADGraph.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ADGraph.Client.ProtectedToken'
